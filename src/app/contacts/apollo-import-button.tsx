...
export function ApolloImportButton() {
  ...
  async function postApolloImport(requestBody: unknown, tag: string) {
    console.log(`[apollo-import-ui][${tag}] request payload`, requestBody);
    ...
    const data = (await res.json().catch(() => null)) as
      ...
      | null;
    console.log(`[apollo-import-ui][${tag}] response`, {
      status: res.status,
      ok: res.ok,
      data,
    });
  ...
  async function handleImport() {
    ...
    try {
      ...
      // Close the modal, then perform a full browser navigation (not a soft
      // router push) to /contacts. A hard nav bypasses Next's RSC router cache,
      // which can otherwise re-render the contacts table from a stale prefetch
      // taken before the import inserted any rows.
      setOpen(false);
      const imported = payload.imported ?? 0;
      const nextUrl = imported > 0 ? `/contacts?imported=${imported}` : "/contacts";
  ...
  return (
    <>
      ...
      <Dialog
        open={open}
        // Only allow the dialog to close via the explicit X button (Base UI
        // reason `close-press`) or our own imperative `setOpen(false)` after
        // an import. Outside-clicks, Escape, and focus-out are ignored so
        // users don't accidentally lose the form mid-edit.
      ...
      >
        <DialogContent className="max-h-[85vh] w-[min(1200px,95vw)] max-w-[min(1200px,95vw)] sm:!max-w-[min(1200px,95vw)] overflow-y-auto text-base">
          <DialogHeader>
            <DialogTitle className="text-xl">Import from Apollo (ICP filters)</DialogTitle>
            <DialogDescription className="text-base">
              Filters are preloaded from your ICP profile. Adjust and import.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setOpen(false)} disabled={loading} className="h-11 px-4 text-base">
              Close
            </Button>
