...
export type ContactsListBasePath = "/contacts" | "/linkedin";

export type ContactsPageSearchParams = {
  q?: string;
  imported?: string;
  showLocked?: string;
  retried?: string;
...
export async function ContactsPageBody({
  basePath,
  searchParams,
}: {
  basePath: ContactsListBasePath;
      ...
      {lockedCount > 0 && !dbError && (
        <div className="mb-4 flex flex-col gap-2 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-950 dark:border-amber-800 dark:bg-amber-950/40 dark:text-amber-100 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex shrink-0 items-center gap-2">
            <RetryEnrichmentButton lockedCount={lockedCount} basePath={basePath} />
            <Link
              href={toggleHref}
      ...
      <Card className="min-w-0">
        ...
        <CardContent className="min-w-0">
          ...
          {contacts.length === 0 && !dbError ? (
            ...
            <>
              <ContactsTable contacts={contacts} basePath={basePath} />
              <ContactsPager
                page={page}
